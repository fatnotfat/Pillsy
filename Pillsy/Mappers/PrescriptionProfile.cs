...
namespace Pillsy.Mappers
{
    public class PrescriptionProfile : Profile
    {
        public PrescriptionProfile()
        {
            ...
            .ForPath(
                dest => dest.Diagnosis,
                opt => opt.MapFrom(src => src.Data.Meta_data.Pathological)
            )
            .ForMember(
                dest => dest.Status,
            ...
            .ForPath(
                dest => dest.CreatedBy,
                opt => opt.MapFrom(src => src.Data.Meta_data.User_id)
            )
            .ForPath(
            ...
            .ForPath(
                ...
                opt => opt.MapFrom(src => src.Data.Meta_data.Modified_at)
            )
            .ForPath(
                dest => dest.PrescriptionID,
                opt => opt.MapFrom(src => src.Data.Medication_records_id)
            )
            .ForPath(
                dest => dest.PatientID,
                opt => opt.MapFrom(src => src.Data.Meta_data.User_id)
